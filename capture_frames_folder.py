...
def main():
    ...
    ap.add_argument("--source", required=False, help="RTSP url or /dev/videoX or file path")
    ...
    ap.add_argument("--crop-frac", type=float, default=0.6, help="Center-crop fraction (1.0=no crop).")
    ap.add_argument("--save-full", action="store_true", help="Also save *_full.jpg")

    # Read-from-folder options
    ap.add_argument("--frames-dir", default="", help="Operate on images from this folder instead of camera")
    ...
    ap.add_argument("--angles-from-name", default=True, action="store_true",
                    help="Parse pose from filename like ..._x..._y..._z..._yaw...")
    ...
    ap.add_argument("--manual-pose", default="", help="Pose used in interactive mode as 'x,y,z,yaw'.")
    ap.add_argument("--suffix", action="store_true", help="Append _0001, _0002… to avoid overwrite.")

    # VLM options
    ap.add_argument("--vlm", default="", help="VLM describe endpoint, e.g. http://172.16.17.12:8080/describe")
    ...
    if not args.frames_dir:
        if not args.source:
            ap.error("When not using --frames-dir you must provide --source")
        cap = open_capture(args.source, args.width, args.height, args.fps, args.fourcc)
        for _ in range(args.warmup):
            cap.read()

    # --- interactive / folder / timer modes ---
    if args.interactive:
        if args.frames_dir:
