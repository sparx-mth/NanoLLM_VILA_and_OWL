...
class OwlPredictor(torch.nn.Module):
    ...
    def extract_rois(self, image: torch.Tensor, rois: torch.Tensor, pad_square: bool = True,
        ...
        batch_size = image.shape[0]  # Define batch_size from the input tensor
        ...
        # extract rois using batch-aware list format
        if batch_size > 1:
            rois_per_image = len(rois) // batch_size
            rois_list = [rois[i * rois_per_image: (i + 1) * rois_per_image] for i in range(batch_size)]
            roi_images = roi_align(image, rois_list, output_size=self.get_image_size())
        else:
            roi_images = roi_align(image, [rois], output_size=self.get_image_size())

        # mask rois
        if pad_square:
            # Match mask to the number of ROIs extracted
            if mask.shape[0] != roi_images.shape[0]:
                mask = mask.repeat(roi_images.shape[0] // mask.shape[0], 1, 1)
        ...
        class Wrapper(torch.nn.Module):
            ...
            def forward(self, image):
                ...
                for start_index in range(0, b, self.max_batch_size):
                    ...
                    current_batch = image_slice.shape[0]
                    if current_batch < self.max_batch_size:
                        padding = torch.zeros(
                            (self.max_batch_size - current_batch, *image_slice.shape[1:]),
                            dtype=image_slice.dtype, device=image_slice.device
                        )
                        image_slice = torch.cat([image_slice, padding], dim=0)

                    output = self.base_module(image_slice)
                    results.append([out[:current_batch] for out in output])
    ...
    def predict(self,
        ...
        if isinstance(image, list):
            # 1. Preprocess all images into one big 4D tensor [4, 3, 768, 768]
            image_tensors = [self.image_preprocessor.preprocess_pil_image(img) for img in image]
            image_tensor = torch.cat(image_tensors, dim=0)

            # 2. Create one ROI per image (the full frame)
            rois = []
            for img in image:
                rois.append([0, 0, img.width, img.height])
            rois = torch.tensor(rois, dtype=image_tensor.dtype, device=image_tensor.device)

            # 3. Use the updated batch-aware encode_rois
            ...
            rois = torch.tensor([[0, 0, image.width, image.height]], dtype=image_tensor.dtype,
