...
def parse_pose_from_name(fname: str):
    m = _POSE_NAME_RE.match(os.path.basename(fname))
    if not m: return None
    return {"x": float(m["x"]), "y": float(m["y"]), "z": float(m["z"]), "yaw": float(m["yaw"])}
...
def main():
    ...
    ap.add_argument("--angles-from-name", action="store_true",
