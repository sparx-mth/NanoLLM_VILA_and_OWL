...
rsync -avz \
      ./scripts/run_nano_owl_container.sh \
...
if [ -z "$RUNNING" ]; then
    ...
    ssh $JETSON_USER@$JETSON_IP "chmod +x $REMOTE_PATH/scripts/run_nano_owl_container.sh && $REMOTE_PATH/scripts/run_nano_owl_container.sh"
else
